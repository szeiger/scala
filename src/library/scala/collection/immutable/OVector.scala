 ...
 */
@SerialVersionUID(3L)
object OVector extends StrictOptimizedSeqFactory[OVector] {

  def empty[A]: OVector[A] = NIL

  def from[E](it: collection.IterableOnce[E]): OVector[E] =
    it match {
      ...
      case _ =>
        ...
        if (knownSize == 0) empty[E]
          ...
          val v = new OVector[E](0, knownSize, 0)
          v.depth = 1
          v.display0 = display0.asInstanceOf[Array[AnyRef]]
          releaseFence()
          v
  ...
  def newBuilder[A]: ReusableBuilder[A, OVector[A]] = new OVectorBuilder[A]

  /** Creates a OVector of one element. Not safe for publication, the caller is responsible for `releaseFence` */
  private def single[A](elem: A): OVector[A] = {
    val s = new OVector[A](0, 1, 0)
    s.depth = 1
  ...
  @transient
  private val NIL = new OVector[Nothing](0, 0, 0)

  private val defaultApplyPreferredMaxLength: Int =
    try System.getProperty("scala.collection.immutable.OVector.defaultApplyPreferredMaxLength",
      "1024").toInt
    catch {
...
/** OVector is a general-purpose, immutable data structure.  It provides random access and updates
 ...
 *  section on `OVectors` for more information.
 *
 ...
 */
final class OVector[+A] private[immutable] (private[collection] val startIndex: Int, private[collection] val endIndex: Int, private[immutable] val focus: Int)
    extends AbstractSeq[A]
        with IndexedSeq[A]
        with IndexedSeqOps[A, OVector, OVector[A]]
        with StrictOptimizedSeqOps[A, OVector, OVector[A]]
        with IterableFactoryDefaults[A, OVector]
        with OVectorPointer[A]
        with DefaultSerializable { self =>

  override def iterableFactory: SeqFactory[OVector] = OVector
  ...
  private[collection] def initIterator[B >: A](s: OVectorIterator[B]): Unit = {
    s.initFrom(this)
  ...
  override def iterator: Iterator[A] = {
    if(isEmpty)
      ...
      val s = new OVectorIterator[A](startIndex, endIndex)
      initIterator(s)
      s
    }
  }
  ...
  override def stepper[S <: Stepper[_]](implicit shape: StepperShape[A, S]): S with EfficientSplit = {
    ...
    var depth = -1
    val displaySource: OVectorPointer[A] =
      if (dirty) iterator.asInstanceOf[OVectorIterator[A]]
      else this
    ...
    val s = shape.shape match {
      case StepperShape.IntShape    => new IntOVectorStepper   (startIndex, endIndex, depth, trunk)
      case StepperShape.LongShape   => new LongOVectorStepper  (startIndex, endIndex, depth, trunk)
      case StepperShape.DoubleShape => new DoubleOVectorStepper(startIndex, endIndex, depth, trunk)
      case _         => shape.parUnbox(new AnyOVectorStepper[A](startIndex, endIndex, depth, trunk))
    }
  ...
  override def updated[B >: A](index: Int, elem: B): OVector[B] = updateAt(index, elem)

  override def take(n: Int): OVector[A] = {
    if (n <= 0)
      OVector.empty
  ...
  override def drop(n: Int): OVector[A] = {
    if (n <= 0)
      this
    else if (startIndex < endIndex - n)
  ...
  override def takeRight(n: Int): OVector[A] = {
    if (n <= 0)
      OVector.empty
  ...
  override def dropRight(n: Int): OVector[A] = {
    if (n <= 0)
      this
    else if (endIndex - n > startIndex)
  ...
  override def init: OVector[A] = {
    if (isEmpty) throw new UnsupportedOperationException("empty.init")
    dropRight(1)
  }

  // appendAll (suboptimal but avoids worst performance gotchas)
  override def appendedAll[B >: A](suffix: collection.IterableOnce[B]): OVector[B] = {
    import OVector.{Log2ConcatFaster, TinyAppendFaster}
        ...
        case suffix: collection.Iterable[B] =>
          suffix.size match {
            ...
            case n if n <= TinyAppendFaster || n < (this.size >>> Log2ConcatFaster) =>
              var v: OVector[B] = this
              for (x <- suffix) v = v :+ x
              v
            case n if this.size < (n >>> Log2ConcatFaster) && suffix.isInstanceOf[OVector[_]] =>
              var v = suffix.asInstanceOf[OVector[B]]
  ...
  override def prependedAll[B >: A](prefix: collection.IterableOnce[B]): OVector[B] = {
        ...
        case prefix: collection.Iterable[B] =>
          prefix.size match {
            case n if n <= TinyAppendFaster || n < (this.size >>> Log2ConcatFaster) =>
              ...
              v
            case n if this.size < (n >>> Log2ConcatFaster) && prefix.isInstanceOf[OVector[_]] =>
              var v = prefix.asInstanceOf[OVector[B]]
  ...
  private[immutable] def updateAt[B >: A](index: Int, elem: B): OVector[B] = {
    val idx = checkRangeConvert(index)
    val s = new OVector[B](startIndex, endIndex, idx)
    s.initFrom(this)
    s.dirty = dirty
  ...
  override def prepended[B >: A](value: B): OVector[B] = {
    val thisLength = length
    val result =
      if (depth == 1 && thisLength < 32) {
        val s = new OVector(0, thisLength + 1, 0)
        s.depth = 1
        ...
        if (startIndex != blockIndex + 32) {
          val s = new OVector(startIndex - 1, endIndex, blockIndex)
          s.initFrom(this)
          s.dirty = dirty
          ...
          if (shift != 0) {
            ...
            if (depth > 1) {
              ...
              val s = new OVector(startIndex - 1 + shift, endIndex + shift, newBlockIndex)
              s.initFrom(this)
              s.dirty = dirty
            ...
            val s = new OVector(startIndex - 1 + move, endIndex + move, newBlockIndex)
            s.initFrom(this)
            s.dirty = dirty
            ...
            val s = new OVector(startIndex - 1, endIndex, newBlockIndex)
            s.initFrom(this)
            s.dirty = dirty
      ...
      } else OVector.single(value)

    releaseFence()
    result
  }

  override def appended[B >: A](value: B): OVector[B] = {
    val thisLength = length
    val result =
      if (depth == 1 && thisLength < 32) {
        val s = new OVector(0, thisLength + 1, 0)
        ...
        if (endIndex != blockIndex) {
          val s = new OVector(startIndex, endIndex + 1, blockIndex)
          s.initFrom(this)
          s.dirty = dirty
          s.gotoPosWritable(focus, blockIndex, focus ^ blockIndex)
          s.display0(lo) = value.asInstanceOf[AnyRef]
          ...
          if (shift != 0) {
            if (depth > 1) {
              ...
              val s = new OVector(startIndex - shift, endIndex + 1 - shift, newBlockIndex)
              s.initFrom(this)
              s.dirty = dirty
            ...
            val s = new OVector(startIndex, endIndex + 1, newBlockIndex)
            s.initFrom(this)
            s.dirty = dirty
            s.gotoFreshPosWritable(newFocus, newBlockIndex, newFocus ^ newBlockIndex)
            s.display0(lo) = value.asInstanceOf[AnyRef]
  ...
  private def dropFront0(cutIndex: Int): OVector[A] = {
    ...
    val s = new OVector(cutIndex - shift, endIndex - shift, blockIndex - shift)
    s.initFrom(this)
    s.dirty = dirty
    s.gotoPosWritable(focus, blockIndex, focus ^ blockIndex)
    s.preClean(d)
  ...
  private def dropBack0(cutIndex: Int): OVector[A] = {
    val blockIndex = (cutIndex - 1) & ~31
    val xor = startIndex ^ (cutIndex - 1)
    val d = requiredDepth(xor)
    val shift = startIndex & ~((1 << (5 * d)) - 1)

    val s = new OVector(startIndex - shift, cutIndex - shift, blockIndex - shift)
    s.initFrom(this)
    s.dirty = dirty
    s.gotoPosWritable(focus, blockIndex, focus ^ blockIndex)
    s.preClean(d)
  ...
  override protected def applyPreferredMaxLength: Int = OVector.defaultApplyPreferredMaxLength
  ...
  override def toVector: Vector[A] = Vector.from(this)

  def toOVector: OVector[A] = this

  override protected[this] def className = "OVector"
}
...
class OVectorIterator[+A](_startIndex: Int, private[this] var endIndex: Int)
   ...
   */
  private[collection] def remainingOVector: OVector[A] = {
    if(!_hasNext) OVector.empty
    else {
      val v = new OVector(blockIndex + lo, endIndex, blockIndex + lo)
      v.initFrom(this)
      v
    }
  }
...
/** A class to build instances of `OVector`.  This builder is reusable. */
final class OVectorBuilder[A]() extends ReusableBuilder[A, OVector[A]] with OVectorPointer[A] {
  ...
  def result(): OVector[A] = {
    val size = this.size
    if (size == 0)
      return OVector.empty
    val s = new OVector[A](0, size, 0) // should focus front or back?
    s.initFrom(this)
...
private[immutable] trait OVectorPointer[+T] {
  ...
  // used
  private[immutable] final def initFrom[U](that: OVectorPointer[U]): Unit = initFrom(that, that.depth)

  private[immutable] final def initFrom[U](that: OVectorPointer[U], depth: Int) = {
    this.depth = depth
    (depth - 1) match {
      case -1 =>
      case 0 =>
